...
;(function( $, window, document, undefined ) {
	...
	var demos = {
		...
		lineCharts: function( target ,response) {
			...
			var  dt2 = [], st = new Date(2017, 09, 30).getTime();
			
			for( var i = 0; i < d2.length; i++ )
			{
					//alert("st total :"+obj.barChatData[i].date_time);
				dt2.push([st * 13+ i * 3600000, parseFloat( (d2[i]).toFixed( 3 ) )]);         
				//13 26 
			}
	...
	$(document).ready(function() {	
			...
			if($.plot) {
	            ...
	            $.ajax({
	                url: "DashboardController.jsp?action=getPieChartData000"
	            }).done(function (response) {
	                  // alert("*******response.length**********:"+response);
	            	demos.pieChart( $('#demo-charts-0000'), response);
	            	
	                });
	            $.ajax({
	                url: "DashboardController.jsp?action=getPieChartObdData222"
	            }).done(function (response) {
	                  // alert("*******response.length chat 7**********:"+response);
	            	demos.pieChart( $('#demo-charts-2222'), response);
	            	
	                });
	            
	            $.ajax({
	                url: "DashboardController.jsp?action=getBarChartData111"
	            }).done(function (response) {
	                  //  alert("*******response.length BarChart**********:"+response);
	            	demos.lineCharts( $('#demo-charts-1111'), response);
	                });
	            
	            $.ajax({
	                url: "DashboardController.jsp?action=getPieChartObdData333"
	            }).done(function (response) {
	                  // alert("*******response.length chat 999**********:"+response);
	            	demos.pieChart( $('#demo-charts-3333'), response);
...
//	            
//	            $.ajax({
//	                url: "DashboardController.jsp?action=getPieChartObdData888"
//	            }).done(function (response) {
//	                  // alert("*******response.length chat 999**********:"+response);
//	            	demos.pieChart( $('#demo-charts-05555555888'), response);
//	            	
//	                });
//	            
